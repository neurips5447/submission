...
class AdversarialModel_Snli(object):
    def __init__(self, model, tokenizer, maxlen, test_bert=False, bert_type="bert"):
    ...
    def query(self, x_p, x_h, device):
        ...
        if self.test_bert:
            if self.bert_type=="bert":
                token = self.tokenizer.encode_plus(x_p+" [SEP] "+x_h, None, add_special_tokens=True, max_length=self.maxlen, pad_to_max_length=True)
            elif self.bert_type=="roberta":
                token = self.tokenizer(x_p+"</s>"+x_h, None, max_length=self.maxlen, pad_to_max_length=True)
...
class AdversarialModel(object):
    def __init__(self, tokenizer, maxlen, test_bert=False, bert_type="bert"):
    ...
    def tokenize(self, x):
        if self.test_bert:
            ...
            if self.bert_type=="bert":
                token = self.tokenizer.encode_plus(x, None, add_special_tokens=True, max_length=self.maxlen, pad_to_max_length=True, truncation=True)
            elif self.bert_type=="roberta":
                token = self.tokenizer(x, None, max_length=self.maxlen, pad_to_max_length=True, truncation=True)
                token["token_type_ids"] = [0 for i in range(len(token["attention_mask"]))]
    ...
    def query(self, x, device, conn_p):
        ...
        if self.test_bert:
            ...
            logit = self.get((text, mask, token_type_ids), conn_p).squeeze(0)
            #self.model(mode="text_to_logit", input=text, bert_mask=mask, bert_token_id=token_type_ids).squeeze(0)

        else:
            token = self.tokenize(x)
            input_vector=torch.from_numpy(token).to(device).to(torch.long)
            logit = self.get((input_vector), conn_p).squeeze(0)
            #logit = self.model(mode="text_to_logit",input=input_vector).squeeze(0)

        p = F.softmax(logit, dim=-1).detach().cpu().numpy()
        return p
...
class GeneticAdversary(Adversary):
    def __init__(self, opt, attack_surface, num_iters=20, pop_size=60, margin_goal=0.5):
    ...
    def perturb(self, words, choices, model, y, device, conn_p):
        if all(len(c) == 1 for c in choices): return words
        good_idxs = [i for i, c in enumerate(choices) if len(c) > 1]
        idx = random.sample(good_idxs, 1)[0]
        x_list = [' '.join(words[:idx] + [w_new] + words[idx+1:])
                for w_new in choices[idx]]
        preds = [model.query(x, device, conn_p) for x in x_list]
    ...
    def run(self, conn_p, model, x, y, device):
        ...
        y = np.argmax(y) 
        # First query the example itself
        orig_pred = model.query(x, device, conn_p)
        if np.argmax(orig_pred) != y :
            conn_p.send(0)
            conn_p.close()
            return 
        ...
        found = False
        population = [self.perturb(words, choices, model, y, device, conn_p)
                        for i in range(self.pop_size)]
        for g in range(self.num_iters):
            best_idx = min(enumerate(population), key=lambda x: x[1][1])[0]
            #print('Iteration %d: %.6f' % (g, population[best_idx][1]))
            ...
            for i in range(1, self.pop_size):
                ...
                parent2 = population[np.random.choice(range(len(population)), p=sample_probs)][0]
                child = [random.sample([w1, w2], 1)[0] for (w1, w2) in zip(parent1, parent2)]
                child_mut, new_p_y = self.perturb(child, choices, model, y, device, conn_p)
